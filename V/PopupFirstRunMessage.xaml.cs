...
public partial class PopupFirstRunMessage : Popup
{
    private static string _FirstRunText =
        ...
        + " - You can view the schedule of planned games without logging in to the app, but if you want to propose gaming sessions, or join existing sessions, or flag games that you'd like to play, you'll need to sign in. Tap the 'person' icon at the top right of the page to set up a new user name ('user handle') and password.\n"
        + " - LobsterConnect can manage schedules for many different events; to pick an event, tap the event name above the schedule table (or tap the 'hamburger' symbol at the top left to open the main app menu, and pick the 'Change Event' option).  The 'TEST Convention' event is available for you experiment with and check out app features so feel free propose gaming sessions, sign-up to games and add games to your wish-list with the TEST Convention selected - and before using the app for real, remember to switch to an actual live gaming event.\n"
