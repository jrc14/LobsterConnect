...
public partial class PopupFirstRunMessage : Popup
{
    private static string _FirstRunText =
        ...
        + " - To see what gaming sessions are planned for the event, scroll around the schedule table. Note that sessions can be OPEN (still looking for extra people to join), FULL (the game's going to be played, but the organiser isn't looking for any more players to join) or ABANDONED (the organiser has decided that the game isn't going to be happening after all). When you tap on an item in the schedule table, you'll be able to see the game details, including a link for chat about the gaming session (if the organiser has set one up). At the bottom you'll see a button with a 'people' icon on it, and maybe some user names next to it; tap that button to sign up (or if you're already signed up, tap on it to remove your sign-up).\n"
        + " - If you want to propose a new gaming session (to play a particular game at a particular time), tap in a blank space in the schedule table (or choose 'Add Session' from the main menu). Then fill in the details of the session. The app comes with a list of 500 popular games, but if your chosen game isn't on the list, tap on the [add a game not on the list below] item at the top of the list, and enter the game details (take care to enter the correct Board Game Geek URL, because it's hard to change this after the game is added).\n"
        + " - Once you've created the session, you'll be prompted to add notes if you like, and then you'll be given the opportunity to link to a chat for discussing sign-ups for this session. For a BGG Geek List link, just paste it from your web browser; for a WhatsApp link, set up a new group (or use an existing one that you own) and copy the 'invitation to group via link' link from it, and paste it into the popup in LobsterConnect.\n"
